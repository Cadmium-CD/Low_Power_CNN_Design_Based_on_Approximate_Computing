...
class MyConv2d(_ConvNd):
    ...
    def mul_eta(self, mul_in_a,mul_in_b):
        '''
        print("aprox mul")
        re = np.multiply(mul_in_a,mul_in_b)
        sign_a = np.where(mul_in_a<0,-1,1)
        sign_b = np.where(mul_in_b<1,-1,1)
        sign = np.multiply(sign_a,sign_b)
        ...
        par_re = np.multiply(par_re,sign)
        par_re = np.cast['int32'](par_re)
