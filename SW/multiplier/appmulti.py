...
def mymulti():
	...
	mask_1 = np.full(shape=mul_in_a.shape,0x7ff0)
	par_a = np.bitwise_and(mul_in_a,mask_1)
	par_b = np.bitwise_and(mul_in_b,mask_1)
	par_re = np.multiply(par_a,par_b)
	
	
	par_a = np.leftshift(mul_in_a,4)
	par_b = np.leftshift(mul_in_b,4)
	par_c = np.bitwise_or(par_a,par_b)
	mask_2 = np.full(shape=mul_in_a.shape,0x80)
	par_c_7 = np.bitwise_and(par_c,mask_2)
	mask_2 = np.rightshift(mask_2,1)
	...
	par_c_2 = np.rightshift(par_c_3,1)
	par_c_1 = np.rightshift(par_c_3,2)
	par_c_0 = np.rightshift(par_c_3,4)
	...
	par_re = np.bitwise_or(par_re,sign)
